...
var tooltip = d3.select('body').append('div').attr('id','tool')
...
function _drawRateChart(div,arg){
    ...
    svg.selectAll('circle')
        ...
        .on('mousemove',function (d) {
            d3.select(this).attr('r','10px');
            let text = '(' +  d.updateTime + ',' + parseFloat(d[arg])/parseFloat(d.confirmedCount) + ')';
            tooltip.html(text).transition()
                .style('left',d3.event.pageX + 20+'px')
                .style('top',d3.event.pageY +20 +'px')
                .style('opacity',1.0);
...
/*        console.log(tmp);
    ...
    svg.selectAll('circle').data(_data).enter()
        ...
        .on('mousemove',function (d) {
            d3.select(this).attr('r','10px');
            let text = '(' +  d.updateTime + ',' + parseFloat(d[arg]) + ')';
            tooltip.html(text).transition()
                .style('left',d3.event.pageX + 20+'px')
                .style('top',d3.event.pageY +20 +'px')
                .style('opacity',1.0);
...
function addCity(data){
    if(Data == null){
        d3.csv('/bootstrap/recourses/DXYArea.csv',d=>{
            DataMap=[];
            //console.log(DataMap);
            initData(d);
            _addCity(data);
    ...
    function _addCity(data){
        let pdata = findProvinceInData(getCurProvinceName());
        mergeCityMap(findCityInProvinceNode(data,pdata),1);
    }
}
//去掉一个市
...
function _singleChooseFunction(data,i,_this){
    ...
    let loc = '/bootstrap/recourses/'+data.properties.name+'.geojson';
    if(flag){
        CityMap.removeAll();
        DataMap = [];
...
function singleChooseMod() {
    chooseType = 1;
    createChinaMap('#index_china_chart','/bootstrap/recourses/china.geoJson');
    DataMap = [];
...
function _doubleChooseFunction(data,i,_this){
    ...
    //console.log(getCurProvinceName());
    let provinceName = getCurProvinceName();

    _data = null;
    for(let i=0;i<ProvinceData.length;i++){
        if(ProvinceData[i].key == provinceName){
            _data = ProvinceData[i].values;
            break;
        }
    }
    let tag = 1;
    if(flag == false) tag = -1;
    mergeCityMap(_data,tag);
/*    for(let i in _data){
        _m = [];
        for (let j in _data[i].values){
            _m.push(_data[i].values[j]);
            //console.log(_m);
        }
        mergeCityMap(_m,tag);
    }*/
...
function _inverseChooseFunction(data,i,_this){
    ...
    //_data = findProvinceInData(getCurProvinceName());
    let tag = -1;
    if(flag == false) tag = 1;
    mergeCityMap(_data,tag);
/*    for(let i in _data){
...
/*    for(let i in Data){
        for(let j in Data[i].values){
            _m = [];
            for(let k in Data[i].values[j].values){
                _m.push(Data[i].values[j].values[k]);
            }
            mergeCityMap(_m,1);
        }
    }*/
    mergeCityMap(NationMap,1);
    console.log(NationMap);
    ...
    function mcreateMap(data) {
       ...
       // console.log(data);
        d3.select(TMD).selectAll('path').data(data.features)
            .enter().append('path').attr('d',geoPath).style('fill',(d,i)=>color(i)).attr('stroke','black')
